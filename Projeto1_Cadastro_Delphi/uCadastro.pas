...
procedure TfrCadastroAluno.btListaClick(Sender: TObject); //Botão lista
 var
  wSexo: String;
  wCurso: String;
...
begin
  if (StrToInt(edIdade.Text) < 14) or (StrToInt(edIdade.Text) >= 60) then //converte e valida a idade
    begin
      edIdade.SetFocus;  //não deixa sair da idade se ela for inadequada
      ShowMessage('Idade inadequada, você não pode proseguir');
    end;
end;
...
begin
  if Length(edNome.Text) < 3  then  //conta a quantidade de letras da palavra e valida
    begin
      edNome.SetFocus; //não deixa sair se o nome tiver menos de 3 letras
...
procedure TfrCadastroAluno.FormShow(Sender: TObject);  //Criação da tela
begin
  ...
  if FileExists(fDiretorio + '\Aluno.txt') then //verifica se o diretorio existe
      wSLAluno.LoadFromFile(fDiretorio + '\Aluno.txt');   //carrega dados já salvos se ouver o diretorio
end;
